...
class BollettaIrenParser:
    ...
    def __getCostoMedioUnitario(self):
        ...
        costoMedioUnitario = self.page0[
                             self.page0.find(costoMedioUnitarioStr) + len(costoMedioUnitarioStr):self.page0.find(
                                 costoMedioUnitarioMatEnerStr)].replace(',','.')
        subFp = self.page0[
                self.page0.find(costoMedioUnitarioMatEnerStr) + len(costoMedioUnitarioMatEnerStr):len(self.page0)]
        costoMedioUnitarioMatEner = subFp[0:subFp.find('Ulteriori')].replace(',','.')
        return {costoMedioUnitarioStr: float(costoMedioUnitario),
                costoMedioUnitarioMatEnerStr: costoMedioUnitarioMatEner}
    ...
    def __getDettaglioCosti(self):
        ...
        spesaMateriaEnergia = float(
            matched.group().replace(spesaMateriaEnergiaStr,'').replace(',','.') if matched != None else 0.0)
        ...
        spesaTrasportoGestioneContatore = float(
            matched.group().replace(spesaTrasportoGestioneContatoreStr,'').replace(',','.') if matched != None else 0.0)
        ...
        speseOneriSistema = float(
            matched.group().replace(speseOneriSistemaStr,'').replace(',','.') if matched != None else 0.0)
        ...
        altroSoggettoIva = float(
            matched.group().replace(altroSoggettoIvaStr,'').replace(',','.') if matched != None else 0.0)
        ...
        totaleImponibile = float(
            matched.group().replace(totaleImponibileStr,'').replace(',','.') if matched != None else 0.0)
        ...
        arrotondamenti = float(
            matched.group().replace(arrotondamentiStr,'').replace(',','.') if matched != None else 0.0)
        dettaglioCosti[arrotondamentiStr] = arrotondamenti

        matched = re.search(totaleBollettaRegExp,self.page0)
        totaleBolletta = float(
            matched.group().replace(totaleBollettaStr,'').replace(',','.') if matched != None else 0.0)
